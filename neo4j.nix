...
in {
  ...
  config =
    let
      ...
      activationScript = pkgs.writeShellScript "activate-neo4j" ''
       # Directories Setup
    ...
    mkIf cfg.enable {
      ...
      systemd.user.services.neo4j = {
        Unit = { Description = "Neo4j Daemon"; };
        Install = { WantedBy = [ "multi-user.target" ]; };
        Service = {
          Environment = [
            "NEO4J_HOME=${cfg.package}/share/neo4j"
            "NEO4J_CONF=${cfg.directories.home}/conf"
            "PATH=$PATH:${pkgs.gnugrep}/bin:${pkgs.bash-completion}/bin:${pkgs.coreutils}/bin"
          ];

          ExecStartPre = "${activationScript}";
      ...
      home.packages = [ cfg.package pkgs.gnugrep pkgs.bash-completion pkgs.coreutils ];
    };
